...
@Injectable()
export class ProcessNewReferrersTask extends BaseTask {
  private readonly taskType = RECURRING_TASKS_TYPES.referringUser;
  ...
  constructor(
    ...
    this.taskType = RECURRING_TASKS_TYPES.referringUser;
  }
  ...
  async processTask(
    ...
    try {
      ...
      this.logger.log({
        level: "info",
        message: `Processing ProcessNewReferrersTask`,
      });
    } catch (err) {
      this.logger.error(`Error in ProcessNewReferrersTask: ${err.message}`);
    }
  }
}
