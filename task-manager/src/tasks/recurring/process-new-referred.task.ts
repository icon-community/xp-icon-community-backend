...
@Injectable()
export class ProcessNewReferredTask extends BaseTask {
  private readonly taskType = RECURRING_TASKS_TYPES.usingReferralCode;
  ...
  constructor(
    ...
    this.taskType = RECURRING_TASKS_TYPES.usingReferralCode;
  }
  ...
  async processTask(
    ...
    try {
      ...
      this.logger.log({
        level: "info",
        message: `Processing ProcessNewReferredTask`,
      });
    } catch (err) {
      this.logger.error(`Error in ProcessNewReferredTask: ${err.message}`);
    }
  }
}
