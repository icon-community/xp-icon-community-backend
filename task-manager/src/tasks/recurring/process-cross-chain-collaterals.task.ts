...
@Injectable()
export class ProcessCrossChainCollateralsTask extends BaseTask {
  private readonly taskType = RECURRING_TASKS_TYPES.depositNativeCrossChain;
  ...
  async processTask(
    ...
    try {
      ...
      this.logger.log({
        level: "info",
        message: `Processing ProcessCrossChainCollateralsTask`,
      });
    } catch (err) {
      this.logger.error(
        `Error in ProcessCrossChainCollateralsTask: ${err.message}`,
      );
    }
  }
}
