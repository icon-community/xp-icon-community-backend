...
export class ProcessLoansTask extends BaseTask {
  private readonly taskType = RECURRING_TASKS_TYPES.mintingBnusdICON;
  ...
  constructor(
    ...
    this.taskType = RECURRING_TASKS_TYPES.mintingBnusdICON;
  }
  ...
  async processTask(
    ...
    try {
      ...
      this.logger.log({
        level: "info",
        message: `Processing ProcessLoansTask`,
      });
    } catch (err) {
      this.logger.error(`Error in ProcessLoansTask: ${err.message}`);
    }
  }
}
